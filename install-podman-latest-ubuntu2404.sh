...
#
# Usage:
#   chmod +x install-podman-latest.sh#!/bin/bash
#===============================================================================
# install-podman-latest.sh
#
# Builds and installs the latest Podman from source on Ubuntu 24.04 LTS
...
log_info() {
    echo -e "${BLUE}[INFO]${NC} $*"
    [[ -d "$BUILD_DIR" ]] && echo "[INFO] $*" >> "$LOG_FILE"
}

log_success() {
    echo -e "${GREEN}[OK]${NC} $*"
    [[ -d "$BUILD_DIR" ]] && echo "[OK] $*" >> "$LOG_FILE"
}

log_warn() {
    echo -e "${YELLOW}[WARN]${NC} $*"
    [[ -d "$BUILD_DIR" ]] && echo "[WARN] $*" >> "$LOG_FILE"
}

log_error() {
    echo -e "${RED}[ERROR]${NC} $*" >&2
    [[ -d "$BUILD_DIR" ]] && echo "[ERROR] $*" >> "$LOG_FILE"
}
